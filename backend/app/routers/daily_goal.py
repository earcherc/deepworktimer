...
from ..schemas import DailyGoalCreate, DailyGoalUpdate, DailyGoal as DailyGoalSchema
...
async def read_daily_goals(skip: int = 0, limit: int = 10, db: AsyncSession = Depends(get_session)):
...
async def update_daily_goal(daily_goal_id: int, daily_goal: DailyGoalUpdate, db: AsyncSession = Depends(get_session)):
    result = await db.execute(select(DailyGoal).where(DailyGoal.id == daily_goal_id))
...
async def delete_daily_goal(daily_goal_id: int, db: AsyncSession = Depends(get_session)):
    result = await db.execute(select(DailyGoal).where(DailyGoal.id == daily_goal_id))
    daily_goal = result.scalar_one_or_none()
    if daily_goal:
